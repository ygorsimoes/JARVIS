...
class SpeechAnalyzerSTT(STTAdapter):
    async def transcribe_stream(self) -> AsyncIterator[str]:
        proc = await asyncio.create_subprocess_exec(
            "./swift/.build/release/speechanalyzer-cli",
...
```swift
// swift/FoundationModelsBridge/main.swift
...
jarvis/
│
├── main.py                          # ResourceGovernor antes de tudo
├── config.py                        # pydantic-settings · carrega .env
├── bus.py                           # event bus asyncio → redis
│
├── core/
...
│   │   └── porcupine.py
│   ├── vad/
│   │   ├── speech_detector.py       # ★ padrão macOS — runtime nativo Apple
│   │   └── silero_vad.py            # fallback cross-platform — CPU only
│   ├── stt/
│   │   ├── speech_analyzer.py       # ★ padrão macOS — Swift CLI · NDJSON
│   │   ├── mlx_whisper.py           # fallback macOS — Metal · Apple Silicon only
│   │   ├── faster_whisper.py        # fallback Linux · Windows
│   │   └── whisper_cpp.py           # fallback universal
│   ├── llm/
│   │   ├── foundation_models.py     # ★ hot path macOS — Swift · sessão stateful
│   │   ├── mlx_lm.py                # deliberativo macOS — max_kv_size obrigatório
│   │   ├── ollama.py                # deliberativo Linux · Windows
│   │   ├── anthropic.py
│   │   └── openai.py
│   ├── tts/
│   │   ├── mlx_audio_kokoro.py      # ★ padrão macOS — Metal · Apple Silicon only
│   │   ├── mlx_audio_qwen3.py       # upgrade macOS — voz clonada
│   │   ├── kokoro_onnx.py           # padrão Linux · Windows — multiplataforma
│   │   ├── piper.py                 # alternativa Linux · Windows — leve e local
│   │   ├── avspeech.py              # fallback macOS
│   │   └── elevenlabs.py
│   └── memory/
│       ├── sqlite_vec.py            # versão pinada
│       └── postgres.py
│
├── memory/
...
| ---------------------- | ----------------------------------------------------------- | ------------------------------ |
| SpeechAnalyzerCLI      | `swift build -c release` em `swift/SpeechAnalyzerCLI/`      | STT + VAD nativo · NDJSON      |
| FoundationModelsBridge | `swift build -c release` em `swift/FoundationModelsBridge/` | Hot path LLM · sessão stateful |
