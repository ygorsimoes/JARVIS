...
class PorcupineActivationAdapter:
    ...
    def _ensure_initialized(self):
        ...
        try:
            ...
            raise RuntimeError(
                "pvporcupine e/ou pyaudio nao instalados. Instale-os com 'pip install pvporcupine pyaudio'."
            ) from exc

        access_key = os.environ.get("PORCUPINE_ACCESS_KEY")
        if not access_key:
            raise RuntimeError(
                "A variavel de ambiente PORCUPINE_ACCESS_KEY nao esta definida."
            )

        self._porcupine = pvporcupine.create(
            access_key=access_key, keywords=[self.keyword]
        )
