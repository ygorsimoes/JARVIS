...
actor ToolInvocationCoordinator {
    ...
    func requestToolCall(name: String, arguments: JSONValue) async throws -> String {
        var logger = makeFoundationModelsLogger()
        logger[metadataKey: "session_id"] = "\(sessionID)"
        ...
        logger.info(
            "Requesting remote tool call",
            metadata: ["tool_name": "\(name)", "call_id": "\(callID)"]
        )
        continuation.yield(
            encodeSSEBuffer(
                SSEEvent(
    ...
    func submitToolResult(callID: String, result: JSONValue) -> Bool {
        ...
        guard let continuation = self.pendingCalls.removeValue(forKey: callID) else {
            logger.warning("Received unknown tool result", metadata: ["call_id": "\(callID)"])
            return false
        }
        logger.info("Tool result accepted", metadata: ["call_id": "\(callID)"])
...
actor BridgeSessionStore {
    ...
    func create(_ payload: SessionCreateRequest) throws -> String {
        ...
        var logger = makeFoundationModelsLogger()
        logger[metadataKey: "session_id"] = "\(sessionID)"
        logger.info("Created bridge session")
        return sessionID
    }
    ...
    func delete(_ sessionID: String) async {
        ...
        var logger = makeFoundationModelsLogger()
        logger[metadataKey: "session_id"] = "\(sessionID)"
        logger.info("Deleting bridge session")
        await session.close()
    }
}
...
struct FoundationModelsBridgeCommand: AsyncParsableCommand {
    ...
    func run() async throws {
        ...
        let logger = makeFoundationModelsLogger()
