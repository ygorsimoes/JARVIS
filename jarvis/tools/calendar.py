...
class CalendarTool:
    ...
    def _ensure_store(self):
        if self._store is None:
            try:
                ...
                raise RuntimeError("pyobjc-framework-EventKit nao esta instalado. Use pip install pyobjc-framework-EventKit.") from exc
        return self._store
    ...
    async def list_events(self, days: int = 1) -> Dict[str, Any]:
        ...
        try:
            ...
            if not granted:
                return {"status": "error", "error": "Permissao ao Calendario negada pelo usuario do macOS."}
            ...
            predicate = self._store.predicateForEventsWithStartDate_endDate_calendars_(now, end_date, None)
            ...
            if events:
                for event in events:
                    result.append({
                        ...
                        "location": str(event.location()) if event.location() else "Sem local"
                    })
